...
class SlideGenerator:
    ...
    def generate_for_all_members(self):
        """모든 멤버의 슬라이드 생성"""
        all_members = list(self.member_stats.keys())
        
        print(f"\n총 {len(all_members)}명 멤버 슬라이드 생성 시작...")
        print(f"멤버 목록: {', '.join(all_members)}\n")
        
        for i, member in enumerate(all_members, 1):
            print(f"\n{'=' * 80}")
            print(f"[{i}/{len(all_members)}] {member} 슬라이드 생성")
            print(f"{'=' * 80}")
            
            self.generate_all_slides(member)
        
        print(f"\n{'=' * 80}")
        print(f"Step 4 완료: 전체 멤버 HTML 슬라이드 생성 성공 ({len(all_members)}명)")
        print(f"{'=' * 80}")
        print(f"\n생성된 슬라이드: {len(all_members) * 5}개 ({len(all_members)}명 × 5페이지)")
        print(f"  - output/slides/ 디렉토리 확인")
        print(f"\n✓ Step 4 HTML 슬라이드 생성 완료. Step 5 (PNG 변환) 준비 완료.")

def main():
    print("=" * 80)
    print("Step 4: HTML 슬라이드 생성 (전체 멤버)")
    print("=" * 80)
    print()
